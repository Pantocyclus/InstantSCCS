...
const defaultHandlers = {
    [LogLevels.INFO]: (message) => {
        kolmafia.printHtml(`<b>[Libram Info]</b> ${message}`);
        kolmafia.logprint(`[Libram] ${message}`);
        return;
    },
    [LogLevels.WARNING]: (message) => {
        kolmafia.printHtml(`<span style="background: orange; color: white;"><b>[Libram Warning]</b> ${message}</span>`);
        kolmafia.logprint(`[Libram] ${message}`);
        return;
    },
    [LogLevels.ERROR]: (error) => {
        kolmafia.printHtml(`<span style="background: red; color: white;"><b>[Libram Error]</b> ${error.toString()}</span>`);
        kolmafia.logprint(`[Libram] ${error}`);
        return;
    },
    [LogLevels.DEBUG]: (message) => {
        kolmafia.printHtml(`<span style="background: red; color: white;"><b>[Libram Debug]</b> ${message}</span>`);
        kolmafia.logprint(`[Libram] ${message}`);
        return;
    },
};
...
const createPropertyGetter = (transform) => (property, default_) => {
    const value = kolmafia.getProperty(property);
...
const getFamiliar = createMafiaClassPropertyGetter(kolmafia.Familiar, kolmafia.toFamiliar, familiarNumericProperties);
const getItem = createMafiaClassPropertyGetter(kolmafia.Item, kolmafia.toItem, itemNumericProperties);
const getLocation = createMafiaClassPropertyGetter(kolmafia.Location, kolmafia.toLocation);
const getMonster = createMafiaClassPropertyGetter(kolmafia.Monster, kolmafia.toMonster, monsterNumericProperties);
const getPhylum = createMafiaClassPropertyGetter(kolmafia.Phylum, kolmafia.toPhylum);
const getStat = createMafiaClassPropertyGetter(kolmafia.Stat, kolmafia.toStat);
/**
...
const handleTypeGetError = (Type, error) => {
    ...
    if (match) {
        kolmafia.print(`${match[0]}; if you're certain that this ${Type.name} exists and is spelled correctly, please update KoLMafia`, "red");
    }
    else {
 ...
 * @category In-game constant
 */
const $effect = createSingleConstant(kolmafia.Effect, kolmafia.toEffect);
/**
 ...
 * @category In-game constant
 */
const $effects = createPluralConstant(kolmafia.Effect);
/**
 * An Element specified by name.
 *
 * @category In-game constant
 */
const $element = createSingleConstant(kolmafia.Element, kolmafia.toElement);
/**
 ...
 * @category In-game constant
 */
const $familiar = createSingleConstant(kolmafia.Familiar, kolmafia.toFamiliar);
/**
 ...
 * @category In-game constant
 */
const $item = createSingleConstant(kolmafia.Item, kolmafia.toItem);
/**
 ...
 * @category In-game constant
 */
const $items = createPluralConstant(kolmafia.Item);
/**
 ...
 * @category In-game constant
 */
const $monsters = createPluralConstant(kolmafia.Monster);
/**
 * A Path specified by name.
 *
 * @category In-game constant
 */
const $path = createSingleConstant(kolmafia.Path, kolmafia.toPath);
/**
 ...
 * @category In-game constant
 */
const $skill = createSingleConstant(kolmafia.Skill, kolmafia.toSkill);
/**
 ...
 * @category In-game constant
 */
const $skills = createPluralConstant(kolmafia.Skill);
/**
 * A Slot specified by name.
 *
 * @category In-game constant
 */
const $slot = createSingleConstant(kolmafia.Slot, kolmafia.toSlot);
/**
 ...
 * @category In-game constant
 */
const $slots = createPluralConstant(kolmafia.Slot);
/**
 * A Stat specified by name.
 *
 * @category In-game constant
 */
const $stat = createSingleConstant(kolmafia.Stat, kolmafia.toStat);
/**
 ...
 * @category In-game constant
 */
const $stats = createPluralConstant(kolmafia.Stat);
/**
 * A Thrall specified by name.
 *
 * @category In-game constant
 */
const $thrall = createSingleConstant(kolmafia.Thrall, kolmafia.toThrall);
/**
...
function have(thing, quantity = 1) {
    ...
    if (thing instanceof kolmafia.Thrall) {
        const thrall = kolmafia.myThrall();
...
function getFoldGroup(item, cache = true) {
    ...
    const result = Object.entries(kolmafia.getRelated(item, "fold"))
        .sort(([, a], [, b]) => a - b)
        .map(([i]) => kolmafia.Item.get(i));
 ...
 */
function totalFamiliarWeight(familiar = kolmafia.myFamiliar(), considerAdjustment = true) {
    return clamp(clamp(kolmafia.familiarWeight(familiar), have($effect `Fidoxene`) ? 20 : 0, Infinity) +
        familiar.soupWeight +
        (considerAdjustment ? kolmafia.weightAdjustment() : 0) +
...
const modeableState = {
    ...
    retrocape: () => kolmafia.getProperty("retroCapeSuperhero") +
        " " +
        kolmafia.getProperty("retroCapeWashingInstructions"),
...
function saveOutfit(name) {
    kolmafia.cliExecute(`outfit save ${name}`);
}
...
function canEquipItemCount() {
    const stats = $stats `Muscle, Mysticality, Moxie`.map((stat) => Math.min(kolmafia.myBasestat(stat), 300));
...
function applyCached(entry, options) {
    ...
    if (outfitName) {
        ...
        if (familiarEquip)
            kolmafia.equip($slot `familiar`, familiarEquip);
    }
    else {
    ...
    if (kolmafia.equippedAmount($item `Crown of Thrones`) > 0 &&
        entry.rider.get($item `Crown of Thrones`)) {
        kolmafia.enthroneFamiliar(entry.rider.get($item `Crown of Thrones`) || $familiar.none);
    }
    if (kolmafia.equippedAmount($item `Buddy Bjorn`) > 0 &&
        entry.rider.get($item `Buddy Bjorn`)) {
        kolmafia.bjornifyFamiliar(entry.rider.get($item `Buddy Bjorn`) || $familiar.none);
    }
...
function verifyCached(entry, warn = true) {
    ...
    for (const slotGroup of slotStructure) {
        ...
        const equippedSet = desiredSlots.map(([slot]) => kolmafia.equippedItem(slot));
    ...
    if (kolmafia.equippedAmount($item `Crown of Thrones`) > 0 &&
        entry.rider.get($item `Crown of Thrones`)) {
        if (entry.rider.get($item `Crown of Thrones`) !== kolmafia.myEnthronedFamiliar()) {
            if (warn) {
    ...
    if (kolmafia.equippedAmount($item `Buddy Bjorn`) > 0 &&
        entry.rider.get($item `Buddy Bjorn`)) {
        if (entry.rider.get($item `Buddy Bjorn`) !== kolmafia.myBjornedFamiliar()) {
            if (warn) {
...
function saveCached(cacheKey, options) {
    ...
    for (const slot of cachedSlots) {
        equipment.set(slot, kolmafia.equippedItem(slot));
    }
    if (kolmafia.equippedAmount($item `card sleeve`) > 0) {
        equipment.set($slot `card-sleeve`, kolmafia.equippedItem($slot `card-sleeve`));
    }
    if (kolmafia.equippedAmount($item `Crown of Thrones`) > 0) {
        rider.set($item `Crown of Thrones`, kolmafia.myEnthronedFamiliar());
    }
    if (kolmafia.equippedAmount($item `Buddy Bjorn`) > 0) {
        rider.set($item `Buddy Bjorn`, kolmafia.myBjornedFamiliar());
    }
    ...
    if (options.onlySlot && options.onlySlot.length > 0) {
        for (const slot of kolmafia.Slot.all()) {
    ...
    const entry = new CacheEntry(equipment, rider, kolmafia.myFamiliar(), canEquipItemCount(), { ...getCurrentModes(), ...options.modes });
...
function maximizeCached(objectives, options = {}) {
    ...
    const cacheKey = [
        objective,
        ...untouchedSlots
            .map((slot) => `${slot}:${kolmafia.equippedItem(slot)}`)
            .sort(),
        have($effect `Offhand Remarkable`),
        options.updateOnLocationChange && kolmafia.myLocation(),
    ].join("; ");
    ...
    const result = kolmafia.maximize(objective, false);
...
function currentCostumes() {
    return new Map(Object.entries(kolmafia.splitModifiers(get("_mummeryMods")))
        .map(([modifier, value]) => {
        ...
        if (!matcher)
            return null;
        return [
            kolmafia.Familiar.get(matcher[2]),
...
const clanIdCache = {};
const toPlayerId = (player) => typeof player === "string" ? kolmafia.getPlayerId(player) : player;
...
class Clan {
    ...
    name;
    static _join(id) {
        const result = kolmafia.visitUrl(`showclan.php?recruiter=1&whichclan=${id}&pwd&whichclan=${id}&action=joinclan&apply=Apply+to+this+Clan&confirm=on`);
    ...
    static getWhitelisted() {
        const page = kolmafia.visitUrl("clan_signup.php");
        return kolmafia.xpath(page, '//select[@name="whichclan"]//option').map((option) => {
            const validHtml = `<select>${option}</select>`;
            const id = Number.parseInt(kolmafia.xpath(validHtml, "//@value")[0]);
            const name = decodeHTML(kolmafia.xpath(validHtml, "//text()")[0]);
     ...
     */
    check() {
        return kolmafia.visitUrl("clan_hall.php").includes(`<b>${this.name}</b>`);
    }
    /**
    ...
    getCurrentFax() {
        this._check();
        const logs = kolmafia.visitUrl("clan_log.php");
    ...
    getRanks() {
        this._check();
        const page = kolmafia.visitUrl("clan_whitelist.php");
        return kolmafia.xpath(page, '//select[@name="level"]//option')
            .map((option) => {
            const validHtml = `<select>${option}</select>`;
            const match = kolmafia.xpath(validHtml, "//text()")[0].match(WHITELIST_DEGREE_PATTERN);
            const id = kolmafia.xpath(validHtml, "//@value")[0];
            if (!match || !id)
                return null;
    ...
    addPlayerToWhitelist(player, rankName, title = "") {
        ...
        if (!rank)
            return false;
        const result = kolmafia.visitUrl(`clan_whitelist.php?action=add&pwd&addwho=${playerId}&level=${rank.id}&title=${title}`);
    ...
    removePlayerFromWhitelist(player) {
        this._check();
        const playerId = toPlayerId(player);
        const result = kolmafia.visitUrl(`clan_whitelist.php?action=updatewl&pwd&who=${playerId}&remove=Remove`);
    ...
    getMeatInCoffer() {
        this._check();
        const page = kolmafia.visitUrl("clan_stash.php");
    ...
    putMeatInCoffer(amount) {
        this._check();
        const result = kolmafia.visitUrl(`clan_stash.php?pwd&action=contribute&howmuch=${amount}`);
    ...
    take(items) {
        ...
        map.forEach((quantity, item) => {
            let needed = Math.max(0, quantity - kolmafia.availableAmount(item));
            if (needed === 0) {
                return map.set(item, 0);
            }
            const foldGroup = getFoldGroup(item);
            for (const foldable of foldGroup) {
                const quantityToFold = Math.min(needed, kolmafia.availableAmount(foldable));
                for (let i = 0; i < quantityToFold; i++) {
                    kolmafia.cliExecute(`fold ${item.name}`);
                    needed--;
                }
            ...
            for (const matchingItem of [item, ...foldGroup]) {
                const quantityToTake = Math.min(needed, kolmafia.stashAmount(matchingItem));
                if (quantityToTake === 0)
                    continue;
                // If we can't take from the stash, there's no sense in iterating through the whole fold group
                if (!kolmafia.takeStash(quantityToTake, matchingItem))
                    return;
                if (matchingItem === item) {
                    ...
                    for (let i = 0; i < quantityToTake; i++) {
                        kolmafia.cliExecute(`fold ${matchingItem.name}`);
                        needed--;
                    }
                }
            }
    ...
    put(items) {
        ...
        map.forEach((quantity, item) => {
            ...
            const returned = Math.min(quantity, kolmafia.availableAmount(item));
...
const statCommunityServicePredictor = (stat) => {
    return () => 60 -
        Math.floor((1 / 30) *
            (kolmafia.myBuffedstat(stat) -
                kolmafia.myBasestat(thralls.get(stat) === kolmafia.myThrall() && !have($effect `Expert Oiliness`)
                    ? $stat `mysticality`
                    : stat)));
};
const visitCouncil = () => kolmafia.visitUrl("council.php");
...
function hypotheticalModifier(modifier, ...effects) {
    ...
    return (kolmafia.numericModifier(modifier) +
        sum(newEffects, (effect) => kolmafia.numericModifier(effect, modifier)));
}
class CommunityService {
     ...
     */
    startTimer() {
        this.timer ??= { time: Date.now(), turns: kolmafia.myTurncount() };
    }
    ...
    static startTimer(name) {
        if (!this.taskTimers.has(name)) {
            this.taskTimers.set(name, { time: Date.now(), turns: kolmafia.myTurncount() });
        }
    }
    ...
    do() {
        if (get("csServicesPerformed").trim().length === 0)
            visitCouncil();
        visitCouncil();
        const councilText = kolmafia.runChoice(this.choice);
    ...
    static printLog(colour = "blue") {
        ...
        for (const [testName, testEntry] of logEntries) {
            ...
            if (type === "test") {
                kolmafia.print(`We predicted the ${testName} test would take ${predictedTurns} turns, ${predictedTurns === turnCost ? "and" : "but"} it took ${turnCost} turns.`, colour);
                kolmafia.print(`${testName} took ${seconds.toFixed(1)} seconds.`, colour);
            }
            else {
                if (!(predictedTurns === 0 && turnCost === 0)) {
                    kolmafia.print(`We predicted the task ${testName} would take ${predictedTurns} turns, ${predictedTurns === turnCost ? "and" : "but"} it took ${turnCost} turns.`, colour);
                }
                kolmafia.print(`The task ${testName} took ${seconds.toFixed(1)} seconds.`, colour);
            }
        }
        const totalTime = sum(logEntries, ([, testEntry]) => testEntry.seconds);
        kolmafia.print(`All together, you have spent ${totalTime.toFixed(1)} seconds during this Community Service run`, colour);
    }
    // Below, we have the tests themselves.
    static HP = new CommunityService(1, "HP", "Donate Blood", () => 60 - Math.floor((kolmafia.myMaxhp() - kolmafia.myBuffedstat($stat `muscle`) - 3) / 30), new Requirement(["HP"], {}));
    ...
    static FamiliarWeight = new CommunityService(5, "Familiar Weight", "Breed More Collies", (...effects) => 60 -
        Math.floor((totalFamiliarWeight(kolmafia.myFamiliar(), false) +
    ...
    static WeaponDamage = new CommunityService(6, "Weapon Damage", "Reduce Gazelle Population", (...effects) => {
        const weaponPower = kolmafia.getPower(kolmafia.equippedItem($slot `weapon`));
        const offhandPower = kolmafia.toSlot(kolmafia.equippedItem($slot `off-hand`)) === $slot `weapon`
            ? kolmafia.getPower(kolmafia.equippedItem($slot `off-hand`))
            : 0;
        const familiarPower = kolmafia.toSlot(kolmafia.equippedItem($slot `familiar`)) === $slot `weapon`
            ? kolmafia.getPower(kolmafia.equippedItem($slot `familiar`))
            : 0;
    ...
    static SpellDamage = new CommunityService(7, "Spell Damage", "Make Sausage", (...effects) => {
        const dragonfishDamage = kolmafia.myFamiliar() === $familiar `Magic Dragonfish`
            ? kolmafia.numericModifier($familiar `Magic Dragonfish`, "Spell Damage Percent", totalFamiliarWeight(), $item.none)
            : 0;
        // We add 0.001 because the floor function sometimes introduces weird rounding errors
        return (60 -
    ...
    static Noncombat = new CommunityService(8, "Non-Combat", "Be a Living Statue", (...effects) => {
        ...
        const currentFamiliarModifier = -1 *
            kolmafia.numericModifier(kolmafia.myFamiliar(), "Raw Combat Rate", totalFamiliarWeight(), kolmafia.equippedItem($slot `familiar`));
        const newFamiliarModifier = -1 *
            kolmafia.numericModifier(kolmafia.myFamiliar(), "Raw Combat Rate", totalFamiliarWeight(kolmafia.myFamiliar(), false) +
                hypotheticalModifier("Familiar Weight", ...effects), kolmafia.equippedItem($slot `familiar`));
    ...
    static BoozeDrop = new CommunityService(9, "Item Drop", "Make Margaritas", (...effects) => {
        const mummingCostume = currentCostumes().get(kolmafia.myFamiliar());
        const mummingBuff = mummingCostume && mummingCostume[0] === "Item Drop"
            ? mummingCostume[1]
            : 0;
        const familiarItemDrop = kolmafia.numericModifier(kolmafia.myFamiliar(), "Item Drop", totalFamiliarWeight(), kolmafia.equippedItem($slot `familiar`)) +
            mummingBuff -
            kolmafia.numericModifier(kolmafia.equippedItem($slot `familiar`), "Item Drop");
        const familiarBoozeDrop = kolmafia.numericModifier(kolmafia.myFamiliar(), "Booze Drop", totalFamiliarWeight(), kolmafia.equippedItem($slot `familiar`)) - kolmafia.numericModifier(kolmafia.equippedItem($slot `familiar`), "Booze Drop");
        // Champagne doubling does NOT count for CS, so we undouble
        const multiplier = kolmafia.haveEquipped($item `broken champagne bottle`) &&
        ...
        return (60 -
            Math.floor((multiplier *
                (hypotheticalModifier("Item Drop", ...effects) -
                    familiarItemDrop -
                    kolmafia.numericModifier(kolmafia.myThrall(), "Item Drop"))) /
                30 +
                0.001) -
    ...
    static HotRes = new CommunityService(10, "Hot Resistance", "Clean Steam Tunnels", (...effects) => {
        const currentFamiliarModifier = kolmafia.numericModifier(kolmafia.myFamiliar(), "Hot Resistance", totalFamiliarWeight(), kolmafia.equippedItem($slot `familiar`));
        const newFamiliarModifier = kolmafia.numericModifier(kolmafia.myFamiliar(), "Hot Resistance", totalFamiliarWeight(kolmafia.myFamiliar(), false) +
            hypotheticalModifier("Familiar Weight", ...effects), kolmafia.equippedItem($slot `familiar`));
        return (60 -
